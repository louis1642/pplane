...
%    tmpdir = tmpdir(1:ll-1);
    ...
    ppset = figure('name','matpplane Setup',...
        ...
        'UserData',ud,'WindowStyle','normal');
    ...
    set(ppset,'MenuBar','figure');  % FIX 2025 -- or ensure matpplane('figdefault',...) does this
    ...
    % FIX 2025
    mefile = findall(ppset, 'Type','uimenu', 'Tag','figMenuFile');
    if isempty(mefile)
        mefile = findMenu(ppset, '^File$');   % handles "&File" too
    end
    meedit = findall(ppset, 'Type','uimenu', 'Tag','figMenuEdit');
    if isempty(meedit)
        meedit = findMenu(ppset, '^Edit$');
    end
    delete(findall(ppset,'Type','uimenu','Tag','figMenuTools'));
    delete(findall(ppset,'Type','uimenu','Tag','figMenuView'));
    delete(findall(ppset,'Type','uimenu','Tag','figMenuInsert'));
    ...
    meexp = findMenu(mefile, '^Export(\s*Setup)?\s*\.\.\.$');      % "&Export...", "Export...", "Export Setup..."
    meprev = findMenu(mefile, '^Print\s*Pre\s*view\s*\.\.\.$');    % "Print Pre&view...", "Print Preview..."
    mepset = findMenu(mefile, '^Pa\s*ge\s*Setup\s*\.\.\.$');       % "Pa&ge Setup..."
    % set(get(mefile,'child'),'vis','off');
    set(allchild(mefile), 'Visible','off');
    ...
    set(allchild(meedit),'Visible','off')
    ...
    set(ppset,'Visible','on','Resize','on');
    set(get(ppset,'Children'),'Visible','on');
    set(allchild(sysmenu),'Visible','on');
    %end
...
function h = findMenu(parent, pattern, varargin)
% Robustly find a uimenu under parent by caption.
% - Works if the caption property is 'Label' or 'Text'
% - Ignores mnemonic ampersands (&)
% - Accepts '...' or '…' endings
% - Accepts case-insensitive regex 'pattern' (string or char)
    if nargin < 3, varargin = {}; end
    ch = allchild(parent);
    if isempty(ch), h = gobjects(0); return; end

    % Try to read Label, fall back to Text
    try
        caps = get(ch, 'Label');
    catch
        caps = get(ch, 'Text');
    end
    if ischar(caps), caps = {caps}; end

    % Normalize captions: remove '&', normalize ellipsis to '...'
    norm = @(s) regexprep(strrep(s,'&',''), '\u2026', '...');
    caps = cellfun(norm, caps, 'uni', 0);

    % Match with case-insensitive regex
    if ~isempty(caps)
        if isstring(pattern), pattern = char(pattern); end
        mask = ~cellfun('isempty', regexp(caps, pattern, 'ignorecase'));
        h = ch(mask);
    else
        h = gobjects(0);
    end

    % Optional extra filter via varargin (e.g., 'Tag', 'xxx')
    if ~isempty(varargin)
        h = findall(h, varargin{:});
    end
end
